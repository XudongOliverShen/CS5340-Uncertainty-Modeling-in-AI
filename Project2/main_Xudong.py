...
def LDA_inference(w, alpha, beta):
    ...
    while True:
        for n in range(N):
        	for i in range(K):
        		phi_new[n][i] = beta[w[n],i] * np.exp(digamma(gamma[i]) - digamma(sum(gamma)))
        	phi_new[n] = phi_new[n] / np.linalg.norm(phi_new[n], ord=1)
...
if __name__ == '__main__':
    ...
    for n in range(3):
        ...
        print('running experiment %d/3' % n)
    ...
    print('alpha_experiment.out saved to current directory.')
